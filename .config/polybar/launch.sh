#!/usr/bin/env bash

# Terminate already running bar instances
killall -q polybar

# Wait until the processes have been shut down
while pgrep -u $UID -x polybar >/dev/null; do sleep 0.1; done

# Set bg color with alpha
COLOR=$(xrdb -query | grep '^\*background:' | cut -f2)
#export POLYBAR_BG="#CC${COLOR#\#}"
export POLYBAR_BG="#FF${COLOR#\#}"


# Launch Polybar (on multiple monitors if detected)
if type "xrandr" >/dev/null 2>&1; then
    IFS=$'\n'  # must set internal field separator to avoid dumb
    for entry in $(xrandr --query | grep " connected"); do
        mon_name=$(cut -d " " -f1 <<< "$entry")
        mon_status=$(cut -d " " -f3 <<< "$entry")

        tray_pos="none"
        if [ "$mon_status" == "primary" ]; then
            tray_pos="center"
        fi

        #MONITOR=$mon_name TRAY_POS=$tray_pos polybar -q -r top 2>&1 | tee -a /tmp/polybar-monitor-"$mon".log & disown
        MONITOR=$mon_name TRAY_POS=$tray_pos polybar -q -r bottom 2>&1 | tee -a /tmp/polybar-monitor-"$mon".log & disown
    done
    unset IFS  # avoid mega dumb by resetting the IFS
else
    #polybar -q -r top 2>&1 | tee -a /tmp/polybar.log & disown
    polybar -q -r bottom 2>&1 | tee -a /tmp/polybar.log & disown
fi
